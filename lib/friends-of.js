...
function friendsOf(adjacencyList, name, distance) {
  if (
    ...
    !Object.keys(adjacencyList).includes(name)
  )
    return undefined;
  ...
  while (distance) {
    ...
    queue.forEach(queuedFriend => {
      // console.log('adjList@queue:', adjacencyList[queuedFriend])
      newQueue.push(...adjacencyList[queuedFriend])
    })

    // console.log('newQueue:', newQueue)
    newQueue.forEach(friend => {
      if(friend !== name) {
        friends.add(friend)
      }
    })
...
// console.log(friendsOf(friendsList, 'Joe', 3))
...
//   adjacencyList[queue.shift()].forEach((friend) => {
